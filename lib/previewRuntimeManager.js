...
class PreviewRuntimeManager {
  constructor({ shell, sendStatus, sendLog }) {
    this.shell = shell;
  ...
  async stopAndWait(key, options = {}) {
    const runtime = this.processes.get(key);
    if (!runtime) {
      this.stop(key, options);
      return;
    }
    ...
    await new Promise((resolve) => {
      ...
      const timer = setTimeout(settle, 1800);
      timer.unref?.();
      runtime.child.once('exit', settle);
      runtime.child.once('close', settle);
      this.stop(key, options);
    });
  }
