...
class PythonPtyTerminalSession extends StdioTerminalSession {
  constructor({ cwd, shell, argv = [], env = process.env }) {
    const bridgeScriptPath = resolvePtyBridgeScriptPath();
    super({
      cwd,
      shell: 'python3',
      env,
      argv: [bridgeScriptPath, shell, ...argv],
    });
  }
}

function resolvePtyBridgeScriptPath() {
  const localPath = path.join(__dirname, 'pty_bridge.py');
  if (!localPath.includes('.asar')) {
    return localPath;
  }

  const unpackedPath = path.join(process.resourcesPath || '', 'app.asar.unpacked', 'lib', 'pty_bridge.py');
  if (unpackedPath && fs.existsSync(unpackedPath)) {
    return unpackedPath;
  }

  const tempPath = path.join(os.tmpdir(), 'pixelbox-pty_bridge.py');
  try {
